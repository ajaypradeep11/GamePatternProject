...
//        ::g_vecMeshes[5].positionXYZ = gTheLights.theLights[0].position;
        ...
        for (unsigned int index = 0; index != ::g_vec_pEnemysShip.size(); index++)
        {
            iGameObject* gameObject = ::g_vec_pEnemysShip[index];
            ((cSpaceWarEnemy*)gameObject)->Update(deltaTime);
            std::vector<cMesh*> enemysSpaceShip = gameObject->getMeshPointers();

            for (unsigned int j = 0; j != enemysSpaceShip.size(); j++)
            {

                glm::mat4 matModel = glm::mat4(1.0f);  // "Identity" ("do nothing", like x1)
           //mat4x4_identity(m);
                DrawObject(enemysSpaceShip.at(j),
                    matModel,
                    matModel_Location,
                    matModelInverseTranspose_Location,
                    program,
...
void loadSpaceWarGame() {
    ...
    cSpaceWarObjects* gameFactory = cSpaceWarObjects::getSpaceWarFactory();


    iGameObject* newGameObject = gameFactory->makeObject("enemy");
    std::cout << newGameObject;
    ::g_vec_pEnemysShip.push_back(newGameObject);
